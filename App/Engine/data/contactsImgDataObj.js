// A class to hold contacts for Stealthy
//
import { BaseDataObj } from './baseDataObj'

const utils = require('./../misc/utils.js')

// TODO: this should go into a file.
//
const defaultImage = '\
data:image/png;base64,'

class ContactsImgDataObj extends BaseDataObj {
  constructor() {
    super()

    // Default Image Dictionary:
    // {
    //   pbj.id: {
    //     default: false,
    //     base64: <base64 image data>,
    //     imageUrl: <anImageUrl>
    //   },
    //   xan.id: {
    //     default: true,
    //     base64: ""
    //   }
    // }
    this.data = {}
  }

  initFromObj(theObj) {
    if (theObj.hasOwnProperty('data') &&
        theObj.hasOwnProperty('time')) {
      if (utils.isEmptyObj(this.data)) {
        super._initFromObj(theObj)
      } else {
        this._mergeWithObj(theObj)
      }
    }
  }

  _mergeWithObj(anObj) {
    if (anObj.hasOwnProperty('data')) {
      for (const key of Object.keys(anObj.data)) {
        // Merge in values we don't already have
        if (!(key in this.data)) {
          this.data[key] = anObj.data[key]
          super.setTimeModified()
        }
      }
    }
  }

  synchronize(anObjectToSynchronize) {
    // TODO: ...
  }

  setContactImg(aContact) {
    if (aContact && aContact.hasOwnProperty('id') && aContact.id) {
      // Set the contact image to the one in aContact if it's available, otherwise
      // if we don't already have an image stored for this contact id, set it
      // to the default one (or a mapping that indicates that so that when we
      // persist this object it's as small as possible).
      if (aContact.hasOwnProperty('base64') && aContact.base64) {
        this.data[aContact.id] = {
          default: false,
          base64: aContact.base64
        }

        if (aContact.hasOwnProperty('image') && aContact.image) {
          this.data[aContact.id].imageURL = aContact.image
        }
      } else if (!(aContact.id in this.data)) {
        this.data[aContact.id] = {
          default: true,
          base64: ""
        }
      }

      super.setTimeModified()
    }
  }

  getContactImg(aContactId) {
    if ((aContactId in this.data) &&
        (!this.data[aContactId].default)) {
      return this.data[aContactId].base64
    }

    return ""     // This makes the user's QR code appear.
    // return defaultImage
  }

  removeContactImg(aContactId) {
    if (aContactId in this.data) {
      delete this.data[aContactId]
      super.setTimeModified()
    }
  }
}

module.exports = { ContactsImgDataObj }
