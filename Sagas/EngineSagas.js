...
function* getToken() {
  const api = DebugConfig.useFixtures ? FixtureAPI
...
function* sendNotificationWorker(action) {
  ...
  const api = DebugConfig.useFixtures ? FixtureAPI : API.notification('https://fcm.googleapis.com/', recepientToken, pk, bearerToken)
